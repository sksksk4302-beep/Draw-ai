...
@app.post("/log-error")
async def log_client_error(error: ErrorLog):
    """
    Receives client-side errors and logs them to the server console (Cloud Logging).
    """
    logger.error(f"🚨 [Client Error] Source: {error.source} | Msg: {error.message}")
    if error.stack:
        logger.error(f"Stack Trace:\n{error.stack}")
    if error.info:
        logger.error(f"Additional Info: {json.dumps(error.info, ensure_ascii=False)}")
